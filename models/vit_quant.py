...
from .ptq import QAct, QConv2d, QLayerNorm, QSoftmax, QLinear
...
class VisionTransformer(nn.Module):
    ...
    def model_quant(self):
        for m in self.modules():
            if type(m) in [QConv2d, QLinear, QAct, QSoftmax]:
                m.quant = True
            if self.cfg.INT_NORM:
                if type(m) in [QLayerNorm]:
                    m.mode = 'int'
...
def deit_tiny_patch16_224(pretrained=False,
    ...
    model = VisionTransformer(
        ...
        mlp_ratio=4,
        qkv_bias=True,
        norm_layer=partial(QLayerNorm, eps=1e-6),
        quant=quant,
        calibrate=calibrate,
        input_quant=True,
        cfg=cfg,
